...
export default function EditAdmin({ navigation }) {
  const [notadmins, setNotAdmins] = useState(null);
  useEffect(() => {
    const unsubscribe = firebase.firestore().collection('users').where("admin","!=","evet").onSnapshot(querySnapshot => {
        const userData2 = [];
  ...
  const collectionRef = firebase.firestore().collection('users').doc(selectedUserId);
    ...
    <>
        ...
        <ListItem containerStyle={{ backgroundColor: "#181818", width:width }}>
          <ListItem.Content style={{ flex: 1, flexDirection: 'row' }}>
            <ListItem.Title style={{ color: "#fff" ,width:width*(12/14)}}>Admin Settings</ListItem.Title>
      ...
      >
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          ...
          <Text style={{ color: '#668', fontSize: 13,width:width*(10/14)}}>Add Admin  : {(selectedItem)}</Text>
        ...
        <View style={{flexDirection:"row",width:width-50}}>
          <Text style={{ color: '#668', fontSize: 13,width:width*(10/14)}}>Remove Admin : {(selectedItem2)}</Text>
