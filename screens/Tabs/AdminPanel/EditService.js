import { View,RefreshControl, SafeAreaView, Dimensions,ImageBackground,Button,Modal, Platform,TouchableOpacity,StyleSheet, TextInput, ScrollView, ViewComponent } from 'react-native'
...
export default function EditService({ navigation }) {
  ...
  useEffect(() => {
    const unsubscribe = firebase.firestore().collection('services').onSnapshot(querySnapshot => {
        const userData2 = [];
  ...
  const collectionRef = firebase.firestore().collection('services').doc(selectedUserId);
  ...
  useEffect(() => {
    const unsubscribe = firebase.firestore().collection('users').where("barber","==","yes").onSnapshot(querySnapshot => {
        const userData = [];
        querySnapshot.forEach(doc => {
    ...
    const getSuggestions2 = useCallback(async q => {
      ...
      if (Array.isArray(admins)) {
        const suggestions2 = admins.filter(item => item.email.toLowerCase().includes(filterToken2)).map(item => ({
          title: item.email,
    ...
    <>
        ...
        <ListItem containerStyle={{ backgroundColor: "#181818", width:width }}>
          <ListItem.Content style={{ flex: 1, flexDirection: 'row' }}>
            <ListItem.Title style={{ color: "#fff" ,width:width*(12/14)}}>Service Settings</ListItem.Title>
...
<SelectDropdown
  data={categories}
  buttonStyle={{borderRadius:10,width:width-50,marginTop:5,justifyContent:"center",flexDirection:"column",alignContent:"center",alignItems:"center",alignSelf:"center",backgroundColor:"#404040"}}
  ...
  buttonTextStyle={{justifyContent:"center",marginTop:12,alignContent:"center",alignItems:"center",alignSelf:"flex-start",textAlign:"center",color:"#909090",fontSize:15}}
...
<Text style={{color:'#909090',textAlign:"center",fontSize:13,paddingLeft:0,fontWeight:"600",width:120}}>Choose Picture</Text>
    ...
    <TouchableOpacity onPress={pickImage}>
        ...
        <AutocompleteDropdown
          ...
          containerStyle={{ flexGrow: 1, flexShrink: 1,justifyContent:"center",width:width-50,alignContent:"center", alignItems:"center", alignSelf:"center",}}
