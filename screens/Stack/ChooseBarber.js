...
import {View,Text,Dimensions,StyleSheet,TouchableOpacity,Platform,SafeAreaView,Image,ScrollView,RefreshControl} from 'react-native';
...
const handleSubmit = async (barber, barberPhoto,barberId) => {
  try {
    ...
    navigation.navigate('ChooseTime', { docum, barberId ,service,barberPhoto,barber});
  } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleBarberPress = (barber, barberPhoto,barberId) => {
      setBarber(barber);
      setBarberPhoto(barberPhoto);
      setBarberId(barberId);
  console.log('Berber with ID:', barberId);
    ...
    const handleBarberPressAndSubmit = async (barber, barberPhoto, barberId) => {
      ...
      console.log('Barber with ID:', barberId);
  ...
  function Item(props) {
    return (
        ...
        <Image style={styles.serviceimg}source={{uri:props.item.image}}/>
          ...
          <TouchableOpacity
          onPress={() => handleSubmit(props.item.fullname, props.item.image,props.item.userId)}
          onPressIn={() => handleBarberPress(props.item.fullname, props.item.image,props.item.userId)}
          style={{backgroundColor:"#181818",marginLeft:10,width:80,height:170,justifyContent:"center",alignSelf:"flex-end",marginRight:30}}
        >
